...
_Static_assert(sizeof((unsigned[]){ VALVE_PINS }) / sizeof(unsigned) == VALVE_COUNT,
               "VALVE_PINS length must match VALVE_COUNT");
_Static_assert(sizeof((const char*[]){ VALVE_NAMES }) / sizeof(const char*) == VALVE_COUNT,
               "VALVE_NAMES length must match VALVE_COUNT");

_Static_assert(MAX_ACTIVE_VALVES > 0, "MAX_ACTIVE_VALVES must be at least 1");
_Static_assert(MAX_ACTIVE_VALVES <= VALVE_COUNT, "MAX_ACTIVE_VALVES cannot exceed VALVE_COUNT");

_Static_assert(MAX_VALVE_ACTIVE_SECONDS > 0, "MAX_VALVE_ACTIVE_SECONDS must be positive");

_Static_assert(LOG_BUFFER_SIZE > 0, "LOG_BUFFER_SIZE must be positive");
